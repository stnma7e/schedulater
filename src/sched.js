...
export default class Schedule {
    constructor(props) {
        ...
        let sched = [];
        app.ports.sched.subscribe((newSched) => {
            this.state = Object.assign(this.state, {
                sched: newSched.map(x => {
    ...
    transform_event_data(start, end, timezone, callback) {
        if (typeof this.state.sched != "undefined" &&
            this.state.sched.length >= 1
        ) {
            let calendar_events = extract_event_data(this.state.sched);
